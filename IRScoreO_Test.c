...
task main()
{
	...
	//Execute IRScoreB with reduced totalTics and some changes to seeking
	clearEncoders();
	wait1Msec(50);
	const int totalTics = 6251;//total tics from before IR to end- CHANGED FOR LESSENED AMOUNT OF FORWARDS
	const int ticsToCenter = 3126;//change once found
	const int ticsToSubtract = 111;//change once tested and found

	//finding IR
	if (abs(nMotorEncoder[leftDrive]) < ticsToCenter){ //If searching buckets 3 and 4
		while (HTIRS2readACDir(IR) > 4)//look for zone 4
			startBackward(20);
		//moveForwardInchesNoReset (20, <INCHES>); <-- USE IF NEEDED
	}
	else{ //If searching buckets 1 and 2 ( zone 4 not found on bucket 3 or 4)
		while (abs(nMotorEncoder[leftDrive]) < totalTics - ticsToSubtract){
			while (HTIRS2readACDir (IR) > 6) //stop at zone 6
				startBackward(20);
			while (HTIRS2readACDir (IR) != 5)//look for zone 5 at a lower speed
				startBackward(15);
		}
		//moveBackwardInchesNoReset (20, <INCHES>); <-- USE IF NEEDED
	}
	//end finding IR
